...
export const InformativeSection = ({ id, eyebrow, title, description, items }: InformativeSectionProps): JSX.Element => {
  return (
    <section
      id={id}
      className="relative overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(15,155,140,0.08),_transparent_55%)] py-24"
    >
      <div className="pointer-events-none absolute inset-x-0 top-0 mx-auto h-40 max-w-5xl rounded-full bg-gradient-to-b from-white/70 to-transparent blur-3xl" />
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-16 grid gap-8 lg:grid-cols-2">
          {items.map((item, index) => (
            <div
              key={item.title}
              className="group relative overflow-hidden rounded-[30px] bg-white/90 p-8 shadow-lg transition duration-300 hover:-translate-y-1 hover:shadow-2xl"
            >
              <div
